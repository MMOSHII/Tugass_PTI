...
class Clouds:
    def __init__(self, cloud_image, count=16) -> None:
        self.clouds = []

        for i in range(count):
            self.clouds.append(Cloud((random.random() * 99999, random.random() * 99999), random.choice(cloud_image), random.random() * 0.5 + 0.5, random.random() * 0.6 + 0.2))
